...
namespace Flames\ErrorHandler;

use ErrorHandler\Inspector\CallableInspectorFactory;
use Flames\ErrorHandler\Exception\ErrorException;
use Flames\ErrorHandler\Handler\CallbackHandler;
use Flames\ErrorHandler\Handler\Handler;
use Flames\ErrorHandler\Handler\HandlerInterface;
use Flames\ErrorHandler\Inspector\InspectorFactory;
use Flames\ErrorHandler\Inspector\InspectorFactoryInterface;
use Flames\ErrorHandler\Inspector\InspectorInterface;
use Flames\ErrorHandler\Util\Misc;
use Flames\ErrorHandler\Util\SystemFacade;
...
final class Run implements RunInterface
{
    ...
    public function register()
    {
        if (!$this->isRegistered) {
            ...
            class_exists("\\Flames\\ErrorHandler\\Exception\\ErrorException");
            class_exists("\\Flames\\ErrorHandler\\Exception\\FrameCollection");
            class_exists("\\Flames\\ErrorHandler\\Exception\\Frame");
            class_exists("\\Flames\\ErrorHandler\\Exception\\Inspector");
            class_exists("\\Flames\\ErrorHandler\\Inspector\\InspectorFactory");
    ...
    /**
     * Unregisters all handlers registered by this ErrorHandler\Run instance.
     *
     * @return Run
     */
    ...
    /**
     * Should ErrorHandler allow Handlers to force the script to quit?
     *
     * @param bool|int $exit
     *
     * @return bool
    ...
    /**
     * Should ErrorHandler send HTTP error code to the browser if possible?
     * ErrorHandler will by default send HTTP code 500, but you may wish to
    ...
    /**
     * Should ErrorHandler exit with a specific code on the CLI if possible?
     * ErrorHandler will exit with 1 by default, but you can specify something else.
     *
     * @param int $code
     *
     * @return int
    ...
    /**
     * Should ErrorHandler push output directly to the client?
    ...
    /**
     * Handles an exception, ultimately generating a ErrorHandler error page.
     *
    ...
    public function handleError($level, $message, $file = null, $line = null)
    {
        if ($level & $this->system->getErrorReportingLevel()) {
            ...
            // Do not propagate errors which were already handled by ErrorHandler.
            return true;
        }
    ...
    private function resolveHandler($handler)
    {
        ...
        if (!$handler instanceof HandlerInterface) {
            throw new InvalidArgumentException(
                ...
                . "ErrorHandler\\Handler\\HandlerInterface"
            );
        }
