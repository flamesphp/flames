...
use Flames\TemplateEngine\Node\BlockReferenceNode;
...
use Flames\TemplateEngine\Node\NodeCaptureInterface;
use Flames\TemplateEngine\Node\NodeOutputInterface;
use Flames\TemplateEngine\Node\PrintNode;
use Flames\TemplateEngine\Node\TextNode;
...
class Parser
{
    ...
    private function filterBodyNodes(Node $node, bool $nested = false): ?Node
    {
        ...
        if (
            ...
            throw new SyntaxError('A template that extends another one cannot include content outside TemplateEngine blocks. Did you forget to put the content inside a {% block %} tag?', $node->getTemplateLine(), $this->stream->getSourceContext());
        }
