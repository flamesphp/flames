...
namespace Flames\TemplateEngine\Test;
...
abstract class IntegrationTestCase extends TestCase
{
     ...
     */
    protected function getTemplateEngineFilters()
    {
        return [];
    }
     ...
     */
    protected function getTemplateEngineFunctions()
    {
        return [];
    }
     ...
     */
    protected function getTemplateEngineTests()
    {
        return [];
    }
    ...
    protected function doIntegrationTest($file, $message, $condition, $templates, $exception, $outputs, $deprecation = '')
    {
        ...
        foreach ($outputs as $i => $match) {
            ...
            foreach ($this->getTemplateEngineFilters() as $filter) {
                $twig->addFilter($filter);
            }

            foreach ($this->getTemplateEngineTests() as $test) {
                $twig->addTest($test);
            }

            foreach ($this->getTemplateEngineFunctions() as $function) {
            ...
            $p->setValue($twig, '__TemplateEngineTemplate_'.hash(\PHP_VERSION_ID < 80100 ? 'sha256' : 'xxh128', uniqid(mt_rand(), true), false).'_');
