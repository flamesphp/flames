...
use Flames\TemplateEngine\Error\LoaderError;
use Flames\TemplateEngine\Error\RuntimeError;
use Flames\TemplateEngine\ExpressionParser;
use Flames\TemplateEngine\Markup;
use Flames\TemplateEngine\Node\Expression\Binary\AddBinary;
use Flames\TemplateEngine\Node\Expression\Binary\AndBinary;
use Flames\TemplateEngine\Node\Expression\Binary\BitwiseAndBinary;
use Flames\TemplateEngine\Node\Expression\Binary\BitwiseOrBinary;
use Flames\TemplateEngine\Node\Expression\Binary\BitwiseXorBinary;
use Flames\TemplateEngine\Node\Expression\Binary\ConcatBinary;
use Flames\TemplateEngine\Node\Expression\Binary\DivBinary;
use Flames\TemplateEngine\Node\Expression\Binary\EndsWithBinary;
use Flames\TemplateEngine\Node\Expression\Binary\EqualBinary;
use Flames\TemplateEngine\Node\Expression\Binary\FloorDivBinary;
use Flames\TemplateEngine\Node\Expression\Binary\GreaterBinary;
use Flames\TemplateEngine\Node\Expression\Binary\GreaterEqualBinary;
use Flames\TemplateEngine\Node\Expression\Binary\HasEveryBinary;
use Flames\TemplateEngine\Node\Expression\Binary\HasSomeBinary;
use Flames\TemplateEngine\Node\Expression\Binary\InBinary;
use Flames\TemplateEngine\Node\Expression\Binary\LessBinary;
use Flames\TemplateEngine\Node\Expression\Binary\LessEqualBinary;
use Flames\TemplateEngine\Node\Expression\Binary\MatchesBinary;
use Flames\TemplateEngine\Node\Expression\Binary\ModBinary;
use Flames\TemplateEngine\Node\Expression\Binary\MulBinary;
use Flames\TemplateEngine\Node\Expression\Binary\NotEqualBinary;
use Flames\TemplateEngine\Node\Expression\Binary\NotInBinary;
use Flames\TemplateEngine\Node\Expression\Binary\OrBinary;
use Flames\TemplateEngine\Node\Expression\Binary\PowerBinary;
use Flames\TemplateEngine\Node\Expression\Binary\RangeBinary;
use Flames\TemplateEngine\Node\Expression\Binary\SpaceshipBinary;
use Flames\TemplateEngine\Node\Expression\Binary\StartsWithBinary;
use Flames\TemplateEngine\Node\Expression\Binary\SubBinary;
use Flames\TemplateEngine\Node\Expression\Filter\DefaultFilter;
use Flames\TemplateEngine\Node\Expression\NullCoalesceExpression;
use Flames\TemplateEngine\Node\Expression\Test\ConstantTest;
use Flames\TemplateEngine\Node\Expression\Test\DefinedTest;
use Flames\TemplateEngine\Node\Expression\Test\DivisiblebyTest;
use Flames\TemplateEngine\Node\Expression\Test\EvenTest;
use Flames\TemplateEngine\Node\Expression\Test\NullTest;
use Flames\TemplateEngine\Node\Expression\Test\OddTest;
use Flames\TemplateEngine\Node\Expression\Test\SameasTest;
use Flames\TemplateEngine\Node\Expression\Unary\NegUnary;
use Flames\TemplateEngine\Node\Expression\Unary\NotUnary;
use Flames\TemplateEngine\Node\Expression\Unary\PosUnary;
use Flames\TemplateEngine\NodeVisitor\MacroAutoImportNodeVisitor;
use Flames\TemplateEngine\Source;
use Flames\TemplateEngine\Template;
...
use Flames\TemplateEngine\TokenParser\ApplyTokenParser;
use Flames\TemplateEngine\TokenParser\BlockTokenParser;
use Flames\TemplateEngine\TokenParser\DeprecatedTokenParser;
use Flames\TemplateEngine\TokenParser\DoTokenParser;
use Flames\TemplateEngine\TokenParser\EmbedTokenParser;
use Flames\TemplateEngine\TokenParser\ExtendsTokenParser;
use Flames\TemplateEngine\TokenParser\FlushTokenParser;
use Flames\TemplateEngine\TokenParser\ForTokenParser;
use Flames\TemplateEngine\TokenParser\FromTokenParser;
use Flames\TemplateEngine\TokenParser\IfTokenParser;
use Flames\TemplateEngine\TokenParser\ImportTokenParser;
use Flames\TemplateEngine\TokenParser\IncludeTokenParser;
use Flames\TemplateEngine\TokenParser\MacroTokenParser;
use Flames\TemplateEngine\TokenParser\SetTokenParser;
use Flames\TemplateEngine\TokenParser\UseTokenParser;
use Flames\TemplateEngine\TokenParser\WithTokenParser;
...
final class CoreExtension extends AbstractExtension
{
    ...
    public function getFilters(): array
    {
        return [
            // formatting filters
            new TemplateEngineFilter('date', [self::class, 'dateFormatFilter'], ['needs_environment' => true]),
            new TemplateEngineFilter('date_modify', [self::class, 'dateModifyFilter'], ['needs_environment' => true]),
            new TemplateEngineFilter('format', [self::class, 'sprintf']),
            new TemplateEngineFilter('replace', [self::class, 'replaceFilter']),
            new TemplateEngineFilter('number_format', [self::class, 'numberFormatFilter'], ['needs_environment' => true]),
            new TemplateEngineFilter('abs', 'abs'),
            new TemplateEngineFilter('round', [self::class, 'round']),

            // encoding
            new TemplateEngineFilter('url_encode', [self::class, 'urlencodeFilter']),
            new TemplateEngineFilter('json_encode', 'json_encode'),
            new TemplateEngineFilter('convert_encoding', [self::class, 'convertEncoding']),

            // string filters
            new TemplateEngineFilter('title', [self::class, 'titleStringFilter'], ['needs_environment' => true]),
            new TemplateEngineFilter('capitalize', [self::class, 'capitalizeStringFilter'], ['needs_environment' => true]),
            new TemplateEngineFilter('upper', [self::class, 'upperFilter'], ['needs_environment' => true]),
            new TemplateEngineFilter('lower', [self::class, 'lowerFilter'], ['needs_environment' => true]),
            new TemplateEngineFilter('striptags', [self::class, 'striptags']),
            new TemplateEngineFilter('trim', [self::class, 'trimFilter']),
            new TemplateEngineFilter('nl2br', [self::class, 'nl2br'], ['pre_escape' => 'html', 'is_safe' => ['html']]),
            new TemplateEngineFilter('spaceless', [self::class, 'spaceless'], ['is_safe' => ['html']]),

            // array helpers
            new TemplateEngineFilter('join', [self::class, 'joinFilter']),
            new TemplateEngineFilter('split', [self::class, 'splitFilter'], ['needs_environment' => true]),
            new TemplateEngineFilter('sort', [self::class, 'sortFilter'], ['needs_environment' => true]),
            new TemplateEngineFilter('merge', [self::class, 'arrayMerge']),
            new TemplateEngineFilter('batch', [self::class, 'arrayBatch']),
            new TemplateEngineFilter('column', [self::class, 'arrayColumn']),
            new TemplateEngineFilter('filter', [self::class, 'arrayFilter'], ['needs_environment' => true]),
            new TemplateEngineFilter('map', [self::class, 'arrayMap'], ['needs_environment' => true]),
            new TemplateEngineFilter('reduce', [self::class, 'arrayReduce'], ['needs_environment' => true]),

            // string/array filters
            new TemplateEngineFilter('reverse', [self::class, 'reverseFilter'], ['needs_environment' => true]),
            new TemplateEngineFilter('length', [self::class, 'lengthFilter'], ['needs_environment' => true]),
            new TemplateEngineFilter('slice', [self::class, 'slice'], ['needs_environment' => true]),
            new TemplateEngineFilter('first', [self::class, 'first'], ['needs_environment' => true]),
            new TemplateEngineFilter('last', [self::class, 'last'], ['needs_environment' => true]),

            // iteration and runtime
            new TemplateEngineFilter('default', [self::class, 'defaultFilter'], ['node_class' => DefaultFilter::class]),
            new TemplateEngineFilter('keys', [self::class, 'getArrayKeysFilter']),
        ];
    }
    ...
    {
        return [
            new TemplateEngineFunction('max', 'max'),
            new TemplateEngineFunction('min', 'min'),
            new TemplateEngineFunction('range', 'range'),
            new TemplateEngineFunction('constant', [self::class, 'constant']),
            new TemplateEngineFunction('cycle', [self::class, 'cycle']),
            new TemplateEngineFunction('random', [self::class, 'random'], ['needs_environment' => true]),
            new TemplateEngineFunction('date', [self::class, 'dateConverter'], ['needs_environment' => true]),
            new TemplateEngineFunction('include', [self::class, 'include'], ['needs_environment' => true, 'needs_context' => true, 'is_safe' => ['all']]),
            new TemplateEngineFunction('source', [self::class, 'source'], ['needs_environment' => true, 'is_safe' => ['all']]),
        ];
    }
    ...
    {
        return [
            new TemplateEngineTest('even', null, ['node_class' => EvenTest::class]),
            new TemplateEngineTest('odd', null, ['node_class' => OddTest::class]),
            new TemplateEngineTest('defined', null, ['node_class' => DefinedTest::class]),
            new TemplateEngineTest('same as', null, ['node_class' => SameasTest::class, 'one_mandatory_argument' => true]),
            new TemplateEngineTest('none', null, ['node_class' => NullTest::class]),
            new TemplateEngineTest('null', null, ['node_class' => NullTest::class]),
            new TemplateEngineTest('divisible by', null, ['node_class' => DivisiblebyTest::class, 'one_mandatory_argument' => true]),
            new TemplateEngineTest('constant', null, ['node_class' => ConstantTest::class]),
            new TemplateEngineTest('empty', [self::class, 'testEmpty']),
            new TemplateEngineTest('iterable', 'is_iterable'),
        ];
    }
     ...
     *
     * @deprecated since TemplateEngine 3.8, to be removed in 4.0 (use the native "is_iterable" function instead)
     *
     * @internal
     */
    ...
    public static function constant($constant, $object = null)
    {
        ...
        if (!\defined($constant)) {
            if ('::class' === strtolower(substr($constant, -7))) {
                throw new RuntimeError(sprintf('You cannot use the TemplateEngine function "constant()" to access "%s". You could provide an object and call constant("class", $object) or use the class name directly as a string.', $constant));
            }
     ...
     * @param string $type              The type of attribute (@see \TemplateEngine\Template constants)
     ...
     *
     * @throws RuntimeError if the attribute does not exist and TemplateEngine is running in strict mode and $isDefinedTest is false
     *
     * @internal
     */
    public static function getAttribute(Environment $env, Source $source, $object, $item, array $arguments = [], $type = /* Template::ANY_CALL */ 'any', $isDefinedTest = false, $ignoreStrictCheck = false, $sandboxed = false, int $lineno = -1)
    {
        ...
        if ($object instanceof Template) {
            throw new RuntimeError('Accessing \TemplateEngine\Template attributes is forbidden.', $lineno, $source);
        }
    ...
    {
        if (!$arrow instanceof \Closure && $env->hasExtension('\Flames\TemplateEngine\Extension\SandboxExtension') && $env->getExtension('\Flames\TemplateEngine\Extension\SandboxExtension')->isSandboxed()) {
