...
use Flames\TemplateEngine\Cache\CacheInterface;
use Flames\TemplateEngine\Cache\FilesystemCache;
use Flames\TemplateEngine\Cache\NullCache;
use Flames\TemplateEngine\Error\Error;
use Flames\TemplateEngine\Error\LoaderError;
use Flames\TemplateEngine\Error\RuntimeError;
use Flames\TemplateEngine\Error\SyntaxError;
use Flames\TemplateEngine\Extension\CoreExtension;
use Flames\TemplateEngine\Extension\EscaperExtension;
use Flames\TemplateEngine\Extension\ExtensionInterface;
use Flames\TemplateEngine\Extension\OptimizerExtension;
use Flames\TemplateEngine\Extension\YieldNotReadyExtension;
use Flames\TemplateEngine\Loader\ArrayLoader;
use Flames\TemplateEngine\Loader\ChainLoader;
use Flames\TemplateEngine\Loader\LoaderInterface;
...
use Flames\TemplateEngine\RuntimeLoader\RuntimeLoaderInterface;
...
class Environment
{
    ...
    private $templateClassPrefix = '__TemplateEngineTemplate_';
     ...
     *           a \TemplateEngine\Cache\CacheInterface implementation,
     ...
     *               as this will be the only supported mode in TemplateEngine 4.0)
     */
     ...
     *
     * @return CacheInterface|string|false A TemplateEngine\Cache\CacheInterface implementation,
     ...
     *
     * @param CacheInterface|string|false $cache A TemplateEngine\Cache\CacheInterface implementation,
     *                                           an absolute path to the compiled templates,
     *                                           or false to disable cache
     */
    public function setCache($cache)
    {
        if (\is_string($cache)) {
            ...
            throw new \LogicException('Cache can only be a string, false, or a \TemplateEngine\Cache\CacheInterface implementation.');
        }
    }
    ...
    public function loadTemplate(string $cls, string $name, ?int $index = null): Template
    {
        ...
        if (!class_exists($cls, false)) {
            ...
            if (!class_exists($cls, false)) {
                ...
                if (!class_exists($cls, false)) {
                    throw new RuntimeError(sprintf('Failed to load TemplateEngine template "%s", index "%s": cache might be corrupted.', $name, $index), -1, $source);
                }
            }
        }
     ...
     *
     * Similar to load() but it also accepts instances of \TemplateEngine\Template and
     * \TemplateEngine\TemplateWrapper, and an array of templates where each is tried to be loaded.
     *
    ...
    /**
     * Returns the runtime implementation of a TemplateEngine element (filter/function/tag/test).
     *
    ...
    public function addFilter(TemplateEngineFilter $filter)
    {
     ...
     * @internal
     */
    public function getFilter(string $name): ?TemplateEngineFilter
    {
    ...
    public function addTest(TemplateEngineTest $test)
    {
     ...
     * @internal
     */
    public function getTest(string $name): ?TemplateEngineTest
    {
        return $this->extensionSet->getTest($name);
    }

    public function addFunction(TemplateEngineFunction $function)
    {
     ...
     * @internal
     */
    public function getFunction(string $name): ?TemplateEngineFunction
    {
