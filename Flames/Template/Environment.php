...
use Flames\Kernel;
use Flames\Template\Cache\CacheInterface;
use Flames\Template\Cache\FilesystemCache;
use Flames\Template\Cache\NullCache;
use Flames\Template\Client\Polyfill;
use Flames\Template\Error\Error;
...
use Flames\Template\Extension\OptimizerExtension;
use Flames\Template\Extension\YieldNotReadyExtension;
use Flames\Template\Loader\ArrayLoader;
use Flames\Template\Loader\ChainLoader;
use Flames\Template\Loader\LoaderInterface;
...
use Flames\Template\RuntimeLoader\RuntimeLoaderInterface;
...
class Environment
{
    ...
    private $templateClassPrefix = '__TemplateTemplate_';
     ...
     *           a \Template\Cache\CacheInterface implementation,
     ...
     *               as this will be the only supported mode in Template 4.0)
     */
     ...
     *
     * @return CacheInterface|string|false A Template\Cache\CacheInterface implementation,
     ...
     *
     * @param CacheInterface|string|false $cache A Template\Cache\CacheInterface implementation,
     *                                           an absolute path to the compiled templates,
     *                                           or false to disable cache
     */
    public function setCache($cache)
    {
        if (\is_string($cache)) {
            ...
            throw new \LogicException('Cache can only be a string, false, or a \Template\Cache\CacheInterface implementation.');
        }
    }
    ...
    public function loadTemplate(string $cls, string $name, ?int $index = null): Template
    {
        ...
        if (!class_exists($cls, false)) {
            ...
            if (!class_exists($cls, false)) {
                ...
                if (!class_exists($cls, false)) {
                    throw new RuntimeError(sprintf('Failed to load Template template "%s", index "%s": cache might be corrupted.', $name, $index), -1, $source);
                }
            }
        }
     ...
     *
     * Similar to load() but it also accepts instances of \Template\Template and
     * \Template\TemplateWrapper, and an array of templates where each is tried to be loaded.
     *
    ...
    /**
     * Returns the runtime implementation of a Template element (filter/function/tag/test).
     *
    ...
    public function addTest(TemplateTest $test)
    {
    ...
    protected static $isCtypePolyfillLoaded = false;
    protected static function loadCtypePolyfill(): void
    {
        if (self::$isCtypePolyfillLoaded === true) {
            return;
        }

        Polyfill::load();
        self::$isCtypePolyfillLoaded = true;
    }
}
