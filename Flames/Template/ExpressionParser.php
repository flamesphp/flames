...
use Flames\Template\Node\Expression\ArrowFunctionExpression;
...
class ExpressionParser
{
    ...
    private function getTestNodeClass(TemplateTest $test): string
    {
        if ($test->isDeprecated()) {
            $stream = $this->parser->getStream();
            $message = sprintf('Template Test "%s" is deprecated', $test->getName());
    ...
    private function getFunctionNodeClass(string $name, int $line): string
    {
        ...
        if ($function->isDeprecated()) {
            $message = sprintf('Template Function "%s" is deprecated', $function->getName());
    ...
    private function getFilterNodeClass(string $name, int $line): string
    {
        ...
        if ($filter->isDeprecated()) {
            $message = sprintf('Template Filter "%s" is deprecated', $filter->getName());
