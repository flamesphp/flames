...
class Dumpper
{
    ...
    public static function __load()
    {
        ...
        if ($config->phar->enabled == true && \Connection::isCommandLineRequest() == false)
        ...
        else
        {
            ...
            if (!\File::exists($cachePath . ".installed"))
            {
                ...
                if (!\File::exists($cachePath . ".installed"))
                { echo \Serialize\Json::encode([error => INTERNAL_SERVER_ERROR, message => "Missing KintDumpper lib."]); exit; }
            }
