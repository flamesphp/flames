...
namespace Render\Front
{
    class View
    {
        ...
        public static function fromNamespace($namespace)
        {
            ...
            $iPath = null;
            if (!stringEx($path)->endsWith(".html") && !stringEx($path)->endsWith(".htm") && !stringEx($path)->endsWith(".xhtml") && !stringEx($path)->endsWith(".hml") && !stringEx($path)->endsWith(".php"))
            {
...
//                $plainHTML = str_replace("/link>", $tagScriptClose, $plainHTML); // test
        ...
        public function getIncludeMetadata()
        {
            ...
            $buffer = $this->getLastBuffer("include");

            // Mount included merge sys
            $this->includedHead       = Arr([ titles => Arr(), metas => Arr(), links => Arr() ]);;
            $this->includedScripts    = Arr();
            $this->includedStyleshets = Arr();
            $this->includedBundles = Arr();
            $this->includedData       = Arr();
            ...
            foreach ($matches[0] as $value)
            {
                ...
                $includes->add([plainHTML => $value, src => $attrSrc, "namespace" => $namespace]);
            }

            // Render & merge or remove
            foreach ($includes as $include)
            {
                $includeView = View::fromNamespace($include->namespace);
                if ($includeView == null)
                { $buffer = stringEx($buffer)->replace($include->plainHTML, ""); continue; }

                $this->includedHead       = $this->includedHead->merge($includeView->getHeadMetadata());
                $this->includedScripts    = $this->includedScripts->merge($includeView->getScriptsMetadata());
                $this->includedStyleshets = $this->includedStyleshets->merge($includeView->getStylesheetsMetadata());
                $this->includedBundles    = $this->includedBundles->merge($includeView->getBundleMetadata());
                $this->includedData       = $this->includedData->merge($includeView->getDataMetadata());

                $cleanBody = $includeView->getCleanBodyHTML();
                $buffer = stringEx($buffer)->replace($include->plainHTML, $cleanBody);
            }

            $this->include = $includes;
            $this->includeData = $buffer;
