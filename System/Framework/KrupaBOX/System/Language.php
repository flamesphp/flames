...
class Language
{
    ...
    {
        $config = \Config::get();
        $pharEnabled = ($config->phar->enabled == true && \Connection::isCommandLineRequest() == false);
        ...
        if ($pharEnabled == false)
        {
            ...
            if (!\File::exists($cachePath . ".installed"))
            {
                ...
                if (!\File::exists($cachePath . ".installed"))
                { echo \Serialize\Json::encode([error => INTERNAL_SERVER_ERROR, message => "Missing Umpirsky lib."]); exit; }
            }
