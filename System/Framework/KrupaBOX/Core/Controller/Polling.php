...
namespace Controller
{
    class Polling extends \Controller
    {
        ...
        public static function onRequest($input)
        {
            ...
            if ($onTimeout->containsKey(timeout) && floatEx($onTimeout->timeout)->toFloat() > 0)
                $timeout = floatEx($onTimeout->timeout)->toFloat();

            $finishTime = (\Time::getCurrent()->get() + $timeout);

            while (\Time::getCurrent()->get() <= $finishTime)
            {
                $data = static::onPolling($input, ($timeout - ($finishTime - \Time::getCurrent()->get())));
