...
namespace KrupaBOX\Internal
{
    class Core
    {
        ...
        public static function load()
        {
            ...
            $cacheFolder = APPLICATION_FOLDER . ".cache";
            if (@file_exists(APPLICATION_FOLDER . "Config/.cache")) {
                $cacheDat = @file_get_contents(APPLICATION_FOLDER . "Config/.cache");
                $cacheDat = str_replace("\\", "/", $cacheDat);
                while(strpos($cacheDat, "//") !== false)
                    $cacheDat = str_replace("//", "/", $cacheDat);
                if (strrpos($cacheDat, "/") != (strlen($cacheDat) - 1))
                    $cacheDat = ($cacheDat . "/");
                $cacheFolder = $cacheDat;
            }
