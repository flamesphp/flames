                ...
                if ($reflector->hasMethod("onMaintence")) {
                    ...
                    if ($method->isStatic() && $method->isPublic())
                    {
                        if ($instanceName::onMaintence($ipAddress, $allowed) === false)
                        {
                            ...
                            \Header::sendHeader();
                            echo \Serialize\Json::encode([error => "UNDER_MAINTENCE", message => "This website is under maintence."]); exit;
                        } else return;
                    }
                ...
                if ($reflector->hasMethod("onConnection")) {
                    ...
                    if ($method->isStatic() && $method->isPublic())
                        if ($instanceName::onConnection($ipAddress) === false)
                        {
                            \Header::setContentType("application/json");
                            \Header::sendHeader();
                            echo \Serialize\Json::encode([error => "INTERNAL_SERVER_ERROR", message => "Please, contact an administrator."]); exit;
                        }
                }
            }
        }
            ...
            if ($isBundle == true)
            {
                ...
                if (\File::exists($packageHashPath)) {
                    if (\Dumpper::isPageDumped() == false)
                        \Header::sendHeader();
                    \File::output($packageHashPath); exit; // Fast output mode
                }
            }
